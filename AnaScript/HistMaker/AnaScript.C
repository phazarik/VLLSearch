...
void AnaScript::SlaveBegin(TTree * /*tree*/)
{
  ...
  /*
  //Test:
  //Printing Scale factors:
  vector<sftxt> testsf = muonIDSF;
  cout << "Printing all entries in muonIDSF:" << endl;
  for (const auto& entry : testsf) {
...
Bool_t AnaScript::Process(Long64_t entry)
{
  ...
  if(GoodEvt){
    ...
    if(_data !=0){//Applying trigger on data
