...
#Example: python3 extractHistsFromTrees.py --jobname tree_2LSS_2017UL_baseline_test_Dec09 --dump hist_2LSS_2017UL_baseline_test_Dec09_mm --channel mm --campaign 2017_UL
...
def extractHistsFromTrees(jobname, channel):
    ...
    for s in samples:
        ...
        indir = os.path.join('../input_files/trees_modified/', jobname)
        outdir = os.path.join('../input_files/hists/', dump)
        ...
        inputfilename  = os.path.join(indir, 'tree_'+s+'.root')
        outputfilename = os.path.join(outdir, 'hst_'+s+'.root')
        ...
        processline = f"root -q -b -l 'processTree.C({arguments})'"
...
extractHistsFromTrees(jobname, channel)
